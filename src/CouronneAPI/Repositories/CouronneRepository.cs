...
namespace CouronneAPI.Repositories
{
    public class CouronneRepository : ICouronneRepository
    {
                ...
                Context.Players.Select(
                    player =>
                        new Player
                        {
                            ...
                            LastName = player.LastName,
                            UserName = player.UserName,
                            Games = GetGames(player.Id),
                            Wins = GetWins(player.Id)
                        }).OrderByDescending(x => x.Wins).ToList();
        }
                ...
                Context.Players.Select(
                    player =>
                        new Player
                        {
                            ...
                            LastName = player.LastName,
                            UserName = player.UserName,
                            Games = GetGamesByMonth(player.Id, month),
                            Wins = GetWinsByMonth(player.Id, month)
                        }).OrderByDescending(x => x.Wins).ToList();
        }
        ...
        {
            return
                Context.Players.Where(player => player.Id == id)
                    .Select(
                        player =>
                            new Player
                            {
